...
var list_top    = y + 80;
var list_bottom = y + sprite_get_height(sprite_index) * image_yscale;
var available_h = list_bottom - list_top;
var item_h      = 70;
visible_count   = floor(available_h / item_h);

// Centraliza o bloco de selectors no espaço disponível
var total_h  = min(visible_count, video_count) * item_h;
var start_y  = list_top + floor((available_h - total_h) / 2);

for (var i = 0; i < min(visible_count, video_count); i++)
{
    var inst = instance_create_layer(x + 4, start_y + i * item_h, BmLayer[Box + 1], obj_troll_vid_selector, {context: i, Layer: Layer, Box: Box + 1});
